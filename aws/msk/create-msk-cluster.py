...
os.system("sudo sed -i 's/subnetA/" + subnetA + "/g' /tmp/msk-cluster.json")
#Update subnetB in msk-template
os.system("sudo sed -i 's/subnetB/" + subnetB + "/g' /tmp/msk-cluster.json")
#Update subnetC in msk-template
os.system("sudo sed -i 's/subnetC/" + subnetC + "/g' /tmp/msk-cluster.json")
#Update securityGroup in msk-template
os.system("sudo sed -i 's/securityGroup/"+ securityGroup + "/g' /tmp/msk-cluster.json")
